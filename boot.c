...
int verify_user(char *name, char *pass) {
    ...
    char saved_name[64] = "";
    char saved_pass[64] = "";
    fgets(saved_name, sizeof(saved_name), f);
    saved_name[strcspn(saved_name, "\n")] = 0;
    fgets(saved_pass, sizeof(saved_pass), f);
    saved_pass[strcspn(saved_pass, "\n")] = 0;
    fclose(f);
    return strcmp(name, saved_name) == 0 && strcmp(pass, saved_pass) == 0;
}
...
int main() {
    ...
    while (1) {
        printf("1 Create new user? y/n\n");
        fgets(input, sizeof(input), stdin);
        input[strcspn(input, "\n")] = 0;
        if (strcmp(input, "y") == 0) {
            printf("1 type username\n");
            fgets(username, sizeof(username), stdin);
            username[strcspn(username, "\n")] = 0;
            char pass[64];
            printf("1 type password\n");
            ...
            if (load_user(username)) {
                ...
                printf("1 No user found. Type y to create one\n");
            }
        } else {
            printf("1 Uongo\n");
        }
